function check_seltzo_two_sum_lemmas!(
    ...
    checker("SELTZO-TwoSum-R1R0-POW2-SGA-X",
        (CLASS_X == R1R0) & (CLASS_Y == POW2) &
        (fx + 1 == ey)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 0, ex + 1, ex - (p - 1), ex + 1), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-POW2-SGA-Y",
        (CLASS_Y == R1R0) & (CLASS_X == POW2) &
        (fy + 1 == ex)
    ) do lemma
    ...
    checker("SELTZO-TwoSum-R1R0-POW2-SGB-X",
        (CLASS_X == R1R0) & (CLASS_Y == POW2) &
        (fx == ey) & (ex == fx + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 1, 1, ex, ey - 1, ex), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-POW2-SGB-Y",
        (CLASS_Y == R1R0) & (CLASS_X == POW2) &
        (fy == ex) & (ey == fy + (p - 1))
    ) do lemma
    ...
    checker("SELTZO-TwoSum-R1R0-POW2-SGC-X",
        (CLASS_X == R1R0) & (CLASS_Y == POW2) &
        (fx + 1 < ey) & (ex + 1 > ey) & (ex == fx + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 1, ex + 1, ey - 1, ey), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-POW2-SGC-Y",
        (CLASS_Y == R1R0) & (CLASS_X == POW2) &
        (fy + 1 < ex) & (ey + 1 > ex) & (ey == fy + (p - 1))
    ) do lemma
