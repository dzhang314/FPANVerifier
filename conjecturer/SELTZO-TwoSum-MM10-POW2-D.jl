function check_seltzo_two_sum_lemmas!(
    checker::LemmaChecker,
    ::Val{MM10},
    ::Val{POW2},
    ::Val{DIFF_SIGN},
    x::SELTZOAbstraction,
    y::SELTZOAbstraction,
    ::Type{T},
) where {T<:AbstractFloat}

    p = precision(T)
    pos_zero = SELTZOAbstraction(+zero(T))
    sx, lbx, tbx, ex, fx, gx = unpack(x, T)
    sy, lby, tby, ey, fy, gy = unpack(y, T)

    checker("SELTZO-TwoSum-MM10-POW2-DE-X",
        (CLASS_X == MM10) & (CLASS_Y == POW2) &
        (ex == ey)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 0, fx, fx - 2, ex - (p - 1)), pos_zero)
    end
    checker("SELTZO-TwoSum-MM10-POW2-DE-Y",
        (CLASS_Y == MM10) & (CLASS_X == POW2) &
        (ey == ex)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 0, fy, fy - 2, ey - (p - 1)), pos_zero)
    end

    checker("SELTZO-TwoSum-MM10-POW2-D1-X",
        (CLASS_X == MM10) & (CLASS_Y == POW2) &
        (ex > ey + 1) & (fx + 1 < ey)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 1, 0, ex - 1, ey - 1, ex - (p - 1)), pos_zero)
    end
    checker("SELTZO-TwoSum-MM10-POW2-D1-Y",
        (CLASS_Y == MM10) & (CLASS_X == POW2) &
        (ey > ex + 1) & (fy + 1 < ex)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 1, 0, ey - 1, ex - 1, ey - (p - 1)), pos_zero)
    end

    checker("SELTZO-TwoSum-MM10-POW2-D1A-X",
        (CLASS_X == MM10) & (CLASS_Y == POW2) &
        (fx + 1 == ey)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 1, 0, ex - 1, fx - 1, ex - (p - 1)), pos_zero)
    end
    checker("SELTZO-TwoSum-MM10-POW2-D1A-Y",
        (CLASS_Y == MM10) & (CLASS_X == POW2) &
        (fy + 1 == ex)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 1, 0, ey - 1, fy - 1, ey - (p - 1)), pos_zero)
    end

    checker("SELTZO-TwoSum-MM10-POW2-D1B-X",
        (CLASS_X == MM10) & (CLASS_Y == POW2) &
        (ex == ey + 1) & (fx + 1 < ey)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 0, ex - 1, fx, ey - (p - 2)), pos_zero)
    end
    checker("SELTZO-TwoSum-MM10-POW2-D1B-Y",
        (CLASS_Y == MM10) & (CLASS_X == POW2) &
        (ey == ex + 1) & (fy + 1 < ex)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 0, ey - 1, fy, ex - (p - 2)), pos_zero)
    end

    checker("SELTZO-TwoSum-MM10-POW2-D2A1-X",
        (CLASS_X == MM10) & (CLASS_Y == POW2) &
        (fx == ey)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 1, ex, fx - 2, fx - 1), pos_zero)
    end
    checker("SELTZO-TwoSum-MM10-POW2-D2A1-Y",
        (CLASS_Y == MM10) & (CLASS_X == POW2) &
        (fy == ex)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 1, ey, fy - 2, fy - 1), pos_zero)
    end

    checker("SELTZO-TwoSum-MM10-POW2-D2A2-X",
        (CLASS_X == MM10) & (CLASS_Y == POW2) &
        (fx == ey + 1)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 1, ex, fx - 1, fx), pos_zero)
    end
    checker("SELTZO-TwoSum-MM10-POW2-D2A2-Y",
        (CLASS_Y == MM10) & (CLASS_X == POW2) &
        (fy == ex + 1)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 1, ey, fy - 1, fy), pos_zero)
    end

    checker("SELTZO-TwoSum-MM10-POW2-DB1-X",
        (CLASS_X == MM10) & (CLASS_Y == POW2) &
        (ex == ey + (p - 1)) & (ex == fx + (p - 3))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 0, ex, fx, fx), pos_zero)
    end
    checker("SELTZO-TwoSum-MM10-POW2-DB1-Y",
        (CLASS_Y == MM10) & (CLASS_X == POW2) &
        (ey == ex + (p - 1)) & (ey == fy + (p - 3))
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 0, ey, fy, fy), pos_zero)
    end

    checker("SELTZO-TwoSum-MM10-POW2-DB2-X",
        (CLASS_X == MM10) & (CLASS_Y == POW2) &
        (ex == ey + p) & (ex == fx + (p - 3))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 0, 0, ex, fx, fx),
            SELTZORange(~sy, 0, 0, ey, ey - p, ey))
    end
    checker("SELTZO-TwoSum-MM10-POW2-DB2-Y",
        (CLASS_Y == MM10) & (CLASS_X == POW2) &
        (ey == ex + p) & (ey == fy + (p - 3))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 0, 0, ey, fy, fy),
            SELTZORange(~sx, 0, 0, ex, ex - p, ex))
    end

end
