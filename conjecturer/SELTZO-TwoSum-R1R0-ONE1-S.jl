function check_seltzo_two_sum_lemmas!(
    checker::LemmaChecker,
    ::Val{R1R0},
    ::Val{ONE1},
    ::Val{SAME_SIGN},
    x::SELTZOAbstraction,
    y::SELTZOAbstraction,
    ::Type{T},
) where {T<:AbstractFloat}

    p = precision(T)
    pos_zero = SELTZOAbstraction(+zero(T))
    sx, lbx, tbx, ex, fx, gx = unpack(x, T)
    sy, lby, tby, ey, fy, gy = unpack(y, T)

    checker("SELTZO-TwoSum-R1R0-ONE1-SE0-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == ey) & (fx + 1 < fy) & (ex < fx + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 1, 0, ex + 1, ex - 1, fx + 1), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-SE0-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == ex) & (fy + 1 < fx) & (ey < fy + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 1, 0, ey + 1, ey - 1, fy + 1), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-SE1-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == ey) & (ex < fy + (p - 2)) & (ex == fx + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 1, 1, ex + 1, ex - 1, fy), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-SE1-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == ex) & (ey < fx + (p - 2)) & (ey == fy + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 1, 1, ey + 1, ey - 1, fx), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-SE2-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == ey) & (fx + 1 == fy)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 1, 0, ex + 1, ex - 1, ex), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-SE2-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == ex) & (fy + 1 == fx)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 1, 0, ey + 1, ey - 1, ey), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S1-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex + 1 > ey) & (fx + 1 < ey) & (ex < fy + (p - 2)) & (fx + 1 > fy)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 0, ex + 1, ey - 1, fy), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S1-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey + 1 > ex) & (fy + 1 < ex) & (ey < fx + (p - 2)) & (fy + 1 > fx)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 0, ey + 1, ex - 1, fx), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S1A-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (fx + 1 == ey) & (ex < fy + (p - 2))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 0, ex + 1, fy, fy), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S1A-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (fy + 1 == ex) & (ey < fx + (p - 2))
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 0, ey + 1, fx, fx), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S1B0-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == fy + (p - 2)) & (fx > fy) & (fx + 1 < ey)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 1, ex + 1, ey - 1, fy + 1), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S1B0-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == fx + (p - 2)) & (fy > fx) & (fy + 1 < ex)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 1, ey + 1, ex - 1, fx + 1), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S1B1-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == fy + (p - 2)) & (fx == fy)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 1, ex + 1, ey - 1, ey), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S1B1-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == fx + (p - 2)) & (fy == fx)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 1, ey + 1, ex - 1, ex), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S1AB-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (fx + 1 == ey) & (ex == fy + (p - 2))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 1, ex + 1, fy, fy + 1), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S1AB-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (fy + 1 == ex) & (ey == fx + (p - 2))
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 1, ey + 1, fx, fx + 1), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S2-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex > ey) & (fx + 1 < fy) & (ex < fx + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 0, ex + 1, ey, fx + 1), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S2-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey > ex) & (fy + 1 < fx) & (ey < fy + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 0, ey + 1, ex, fy + 1), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S2A-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex < fy + (p - 2)) & (ex > ey) & (ex == fx + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 1, ex + 1, ey, fy), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S2A-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey < fx + (p - 2)) & (ey > ex) & (ey == fy + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 1, ey + 1, ex, fx), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S2B-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex > ey) & (fx + 1 == fy)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 0, 0, ex + 1, ey, ey), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S2B-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey > ex) & (fy + 1 == fx)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 0, 0, ey + 1, ex, ex), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S3-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex > fy + (p - 2)) & (fx + 1 < ey) & (ex < fx + (p - 1))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 0, 0, ex + 1, ey - 1, fx + 1),
            SELTZORange(sy, 0, 0, fy, fy - p, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S3-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey > fx + (p - 2)) & (fy + 1 < ex) & (ey < fy + (p - 1))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 0, 0, ey + 1, ex - 1, fy + 1),
            SELTZORange(sx, 0, 0, fx, fx - p, fx))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S3A-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (fx + 1 == ey) & (ex > fy + (p - 2))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 0, 0, ex + 1, ex - (p - 1), ex + 1),
            SELTZORange(sy, 0, 0, fy, fy - p, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S3A-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (fy + 1 == ex) & (ey > fx + (p - 2))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 0, 0, ey + 1, ey - (p - 1), ey + 1),
            SELTZORange(sx, 0, 0, fx, fx - p, fx))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S3B0-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex < ey + (p - 2)) & (ex > fy + (p - 1)) & (ex == fx + (p - 1))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 0, 1, ex + 1, ey - 1, ey),
            SELTZORange(sy, 0, 0, fy, fy - p, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S3B0-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey < ex + (p - 2)) & (ey > fx + (p - 1)) & (ey == fy + (p - 1))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 0, 1, ey + 1, ex - 1, ex),
            SELTZORange(sx, 0, 0, fx, fx - p, fx))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S3B1-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == fy + (p - 1)) & (ex == fx + (p - 1))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 0, 0, ex + 1, ey, ey),
            SELTZORange(~sy, 0, 0, fx, fx - p, fx))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S3B1-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == fx + (p - 1)) & (ey == fy + (p - 1))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 0, 0, ey + 1, ex, ex),
            SELTZORange(~sx, 0, 0, fy, fy - p, fy))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S4-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex < ey + (p - 1)) & (ex > fy + (p - 1)) & (fx > ey)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 1, 0, ex, fx, ey),
            SELTZORange(sy, 0, 0, fy, fy - p, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S4-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey < ex + (p - 1)) & (ey > fx + (p - 1)) & (fy > ex)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 1, 0, ey, fy, ex),
            SELTZORange(sx, 0, 0, fx, fx - p, fx))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S4A-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (fx == ey) & (ex < ey + (p - 1)) & (ex > fy + (p - 1))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 1, 0, ex, ey - 1, ey),
            SELTZORange(sy, 0, 0, fy, fy - p, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S4A-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (fy == ex) & (ey < ex + (p - 1)) & (ey > fx + (p - 1))
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 1, 0, ey, ex - 1, ex),
            SELTZORange(sx, 0, 0, fx, fx - p, fx))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S4B-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == fy + (p - 1)) & (fx > ey)
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 1, 1, ex, fx, fy + 1), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S4B-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == fx + (p - 1)) & (fy > ex)
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 1, 1, ey, fy, fx + 1), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-S4AB-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (fx == ey) & (ex == fy + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sx, 1, 1, ex, ey - 1, fy + 1), pos_zero)
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-S4AB-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (fy == ex) & (ey == fx + (p - 1))
    ) do lemma
        add_case!(lemma, SELTZORange(sy, 1, 1, ey, ex - 1, fx + 1), pos_zero)
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-SB10-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == ey + (p - 1)) & (fx > ey)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 1, 1, ex, fx, ey + 1),
            SELTZORange(sy, 0, 0, fy, fy - p, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-SB10-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == ex + (p - 1)) & (fy > ex)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 1, 1, ey, fy, ex + 1),
            SELTZORange(sx, 0, 0, fx, fx - p, fx))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-SB11-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == ey + (p - 1)) & (fx == ey)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 1, 1, ex, ey - 1, ex),
            SELTZORange(sy, 0, 0, fy, fy - p, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-SB11-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == ex + (p - 1)) & (fy == ex)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 1, 1, ey, ex - 1, ey),
            SELTZORange(sx, 0, 0, fx, fx - p, fx))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-SB20-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == ey + p) & (fx > ey + 1)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 1, 1, ex, fx, ey + 2),
            SELTZORange(~sy, 1, 0, ey - 1, fy - 1, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-SB20-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == ex + p) & (fy > ex + 1)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 1, 1, ey, fy, ex + 2),
            SELTZORange(~sx, 1, 0, ex - 1, fx - 1, fx))
    end

    checker("SELTZO-TwoSum-R1R0-ONE1-SB21-X",
        (CLASS_X == R1R0) & (CLASS_Y == ONE1) &
        (ex == ey + p) & (fx == ey + 1)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sx, 1, 1, ex, ey, ex),
            SELTZORange(~sy, 1, 0, ey - 1, fy - 1, fy))
    end
    checker("SELTZO-TwoSum-R1R0-ONE1-SB21-Y",
        (CLASS_Y == R1R0) & (CLASS_X == ONE1) &
        (ey == ex + p) & (fy == ex + 1)
    ) do lemma
        add_case!(lemma,
            SELTZORange(sy, 1, 1, ey, ex, ey),
            SELTZORange(~sx, 1, 0, ex - 1, fx - 1, fx))
    end

end
